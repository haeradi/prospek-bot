...
function humanDelay(minutes = null) {
  ...
  if (minutes !== null) {
    ...
    baseMin = HUMAN_DELAY_POOL_MIN[Math.floor(Math.random() * HUMAN_DELAY_POOL_MIN.length)];
  }
  // ±40% random jitter biar tidak sama terus
  const jitter = baseMin * 0.4;
  const actualMin = (baseMin - jitter/2) + Math.random() * jitter;
  const ms = Math.round(actualMin * 60 * 1000);
  const minVal = (ms / 60000).toFixed(1);
  console.log(`Human delay: ${minVal} menit (${ms}ms)`);
  return ms;
}
