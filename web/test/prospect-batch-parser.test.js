...
test('CSV/TXT diparse bounded, HP dinormalisasi, dan duplikat ditandai',()=>{
 const input='level,nama,hp,motor,nik,alamat,provinsi,kabupaten,kecamatan,desa,rt,rw,source,pekerjaan\nLOW,Ani,0812-3456-7890,,,,,,,,1,Wiraswasta\nMEDIUM,Budi,628123456791,NE0B,,Jl Mawar,KALIMANTAN TIMUR,PENAJAM PASER UTARA,PENAJAM,PENAJAM,001,001,2,Karyawan Swasta\nLOW,Ani Dua,081234567890,,,,,,,,1,Wiraswasta';
...
test('parser menolak oversized, terlalu banyak baris, kolom berbahaya, dan data level invalid',()=>{
 ...
 const missing=parseProspectBatch(Buffer.from('level,nama,hp,motor,nik,alamat,provinsi,kabupaten,kecamatan,desa\nMEDIUM,Budi,081234567890,LY2,,Jalan,KALIMANTAN TIMUR,PENAJAM PASER UTARA,PENAJAM,'),{format:'csv'});assert.deepEqual(missing.errors,[{row:2,code:'INVALID_ROW',field:'desa',reason:'REQUIRED_FOR_MEDIUM_HOT'}])
});
...
test('header wilayah Indonesia dipetakan dan label dinormalisasi',()=>{const out=parseProspectBatch(Buffer.from('level,nama,hp,motor,alamat,provinsi,kabupaten_kota,kecamatan,kelurahan\nMEDIUM,Aceh Satu,081234567890,NE0B,Jalan, Prov. aceh , KOTA  BANDA ACEH , Kec. BAITURRAHMAN , kelurahan PEUNITI '));assert.deepEqual(out.errors,[]);assert.deepEqual([out.rows[0].province,out.rows[0].district,out.rows[0].subDistrict,out.rows[0].village],['ACEH','BANDA ACEH','BAITURRAHMAN','PEUNITI'])});
test('MEDIUM/HOT mewajibkan empat label wilayah',()=>{const out=parseProspectBatch(Buffer.from('level,nama,hp,motor,alamat,provinsi,kota,kecamatan,desa\nMEDIUM,Aceh Satu,081234567890,NE0B,Jalan,ACEH,,BAITURRAHMAN,PEUNITI'));assert.deepEqual(out.errors,[{row:2,code:'INVALID_ROW',field:'kabupaten_kota',reason:'REQUIRED_FOR_MEDIUM_HOT'}])});
