...
test('Data ASSIST Admin mengikuti Sales aktif untuk Prospect Leads dan Activity',()=>{const html=fs.readFileSync(path.join(__dirname,'../public/index.html'),'utf8'),js=fs.readFileSync(path.join(__dirname,'../public/app.js'),'utf8');assert.match(html,/id="adminAssistDataNav"/);assert.match(js,/activeSalesId.*assist-data/s);assert.match(js,/activeSalesId.*assist-activities/s)});

test('dashboard tiga kartu tidak mengakses elemen Draft Lokal yang sudah dihapus',()=>{const html=fs.readFileSync(path.join(__dirname,'../public/index.html'),'utf8'),js=fs.readFileSync(path.join(__dirname,'../public/app.js'),'utf8');assert.doesNotMatch(html,/id="draftCount"/);assert.doesNotMatch(js,/draftCount/)});
