...
test('Admin dapat memilih Sales dan menghubungkan ASSIST pada Sales aktif',()=>{const html=fs.readFileSync(require('path').join(__dirname,'../public/index.html'),'utf8'),js=fs.readFileSync(require('path').join(__dirname,'../public/app.js'),'utf8');for(const id of ['activeSalesSelect','activeSalesConnectForm','dashboardProspects','dashboardDeals','dashboardLost','dashboardLeads'])assert.match(html,new RegExp(`id=\"${id}\"`));assert.match(js,/assist-login-jobs/);assert.match(js,/activeSalesGeneration/);for(const hard of ['12 Unit','48 Prospek','8 Leads'])assert.doesNotMatch(html,new RegExp(hard))});
