...
test('XLSX format bot dipetakan per baris HOT MEDIUM LOW dan nilai tanggal normal tidak dianggap executable',()=>{const out=parseProspectXlsxRows([['Jenis Sales','kode asal prospek','Nama','Nomor HP','Alamat','Provinsi','Kabupaten','Kecamatan','Kelurahan','tipe motor','Nomor NIK','Catatan'],['individu',3,'Hot','081234567891','Jl Hot','Kalimantan Timur','Penajam Paser Utara','Penajam','Petung','VARIO','1234567890123456',new Date('2026-08-30T00:00:00Z')],['individu',3,'Medium','081234567892','Jl Uji','Kalimantan Timur','Penajam Paser Utara','Penajam','Petung','VARIO','',''],['individu',3,'Low','081234567893','','','','','','','','']]);assert.equal(out.rows.length,3);assert.deepEqual(out.rows.map(x=>x.level),['HOT','MEDIUM','LOW']);assert.equal(out.rows[0].sourceCode,'3');assert.equal(out.rows[0].phone,'6281234567891')});
