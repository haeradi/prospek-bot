...
let fakeLoginRunner,runnerDelay,fakeReadClient,readCalls,fakeMutationClient,mutationCalls;
test.beforeEach(async()=>{ runnerDelay=15;readCalls=[];mutationCalls=[];fakeMutationClient={createProspect:async(_token,payload)=>{mutationCalls.push(['create',payload]);return{id:'star-created',prospectNumber:'H704-NEW-1',status:'PROSPECT'}},updateProspectStatus:async(_token,payload)=>{mutationCalls.push(['update',payload]);return{id:payload.prospectId,status:payload.toStatus}}};fakeReadClient={listProspects:async token=>{readCalls.push({kind:'prospects',token});return[{id:'star-p1',prospectNumber:'H704-PRS-1',name:'Customer STAR',phone:'628123456789',status:'HOT',createdAt:'2026-08-28T00:00:00Z',nik:'6401010101010001',address:'Alamat Customer',description:null,motor:'PCX'}]},findProspectsByPhone:async(token,phone)=>{readCalls.push({kind:'prospect-phone',token});return phone==='628123456789'?[{id:'star-p1',prospectNumber:'H704-PRS-1',name:'Customer STAR',phone,status:'HOT',createdAt:'2026-08-28T00:00:00Z'}]:[]},listLeads:async(token,type)=>{readCalls.push({kind:'leads',token,type});return[{assignmentId:`a-${type}`,leadId:'l1',name:`Lead ${type}`,phone:'628199999999',isOverdue:false,isOnTrack:type==='ON_TRACK'}]}};fakeLoginRunner=async({emit,signal})=>{emit({status:'MFA_REQUIRED',mfaNumber:'42'});await new Promise((resolve,reject)=>{const t=setTimeout(resolve,runnerDelay);signal.addEventListener('abort',()=>{clearTimeout(t);reject(Object.assign(new Error('aborted'),{name:'AbortError'}))},{once:true})});return{accessToken:'synthetic-worker-access-token',refreshToken:'synthetic-worker-refresh-token',subjectId:'worker-subject',displayName:'Sales Worker',email:'worker@example.invalid',expiresAt:'2030-01-01T00:00:00.000Z'}};app=createApp({dbPath:':memory:',adminEmail:'admin@example.invalid',adminPassword:'AdminPassword!123',assistMasterKey:Buffer.alloc(32,7).toString('base64'),assistLoginRunner:fakeLoginRunner,assistReadClient:fakeReadClient,assistMutationClient:fakeMutationClient,assistFollowupClient:{followupLead:async()=>({ok:true}),followupProspect:async(_token,payload)=>{mutationCalls.push(['followup',payload]);return{id:'fu-created'}}},assistLoginTimeoutMs:500,parityMutationEnabled:true}); server=app.server; await new Promise(r=>server.listen(0,'127.0.0.1',r)); base=`http://127.0.0.1:${server.address().port}`; });
test.afterEach(async()=>{ await new Promise(r=>server.close(r)); await app.close(); });

test('startup recovery selesai sebelum ready dan parity OFF tidak replay CONFIRMED',async()=>{
 let release;const recovery=new Promise(r=>{release=r}),isolated=createApp({dbPath:':memory:',adminEmail:'startup@example.invalid',adminPassword:'AdminPassword!123',assistMasterKey:Buffer.alloc(32,4).toString('base64'),parityMutationEnabled:false,startupHooks:{followup:()=>recovery,bulk:async()=>[]}});let settled=false;isolated.ready.then(()=>{settled=true});await new Promise(r=>setImmediate(r));assert.equal(settled,false);release([]);await isolated.ready;assert.equal(settled,true);assert.equal(isolated.startupState.ready,true);await isolated.close()
});

test('file-backed DB lama dimigrasikan dan data tetap dapat dibaca',async()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'portal-old-db-')),dbPath=path.join(dir,'portal.db'),{DatabaseSync}=require('node:sqlite'),old=new DatabaseSync(dbPath);old.exec(`CREATE TABLE prospects(id TEXT PRIMARY KEY,owner_id TEXT NOT NULL,customer_name TEXT NOT NULL,phone TEXT NOT NULL,level TEXT NOT NULL,motor TEXT,province TEXT,district TEXT,sub_district TEXT,village TEXT,status TEXT NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL);`);old.prepare('INSERT INTO prospects VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)').run('old-1','owner-1','Old Customer','08123456789','LOW',null,null,null,null,null,'DRAFT',new Date().toISOString(),new Date().toISOString());old.close();const migrated=createApp({dbPath,adminEmail:'migration@example.invalid',adminPassword:'AdminPassword!123',assistMasterKey:Buffer.alloc(32,5).toString('base64')});await migrated.ready;const row=migrated.db.prepare('SELECT customer_name,nik,address,source_code,occupation FROM prospects WHERE id=?').get('old-1');assert.equal(row.customer_name,'Old Customer');assert.equal(row.nik,null);assert.equal(row.address,null);await migrated.close();fs.rmSync(dir,{recursive:true,force:true})
});

test('close membatalkan worker parity dan menunggu worker selesai sebelum DB ditutup',async()=>{
 let started,release;const began=new Promise(r=>{started=r}),blocked=new Promise(r=>{release=r}),isolated=createApp({dbPath:':memory:',adminEmail:'lifecycle@example.invalid',adminPassword:'AdminPassword!123',assistMasterKey:Buffer.alloc(32,6).toString('base64'),shutdownTimeoutMs:500});isolated.services.lifecycle.track('owner-x','followup',async signal=>{started();await new Promise(resolve=>signal.addEventListener('abort',resolve,{once:true}));await blocked});await began;let closed=false;const closing=isolated.close().then(()=>{closed=true});await new Promise(r=>setTimeout(r,10));assert.equal(closed,false);release();await closing;assert.equal(closed,true)
});

test('timeout drain gagal terlihat dan tidak menutup DB saat worker parity belum selesai',async()=>{
 let started;const began=new Promise(r=>{started=r}),isolated=createApp({dbPath:':memory:',adminEmail:'timeout@example.invalid',adminPassword:'AdminPassword!123',assistMasterKey:Buffer.alloc(32,8).toString('base64'),shutdownTimeoutMs:20});isolated.services.lifecycle.track('owner-x','followup',async()=>{started();await new Promise(()=>{})});await began;await assert.rejects(()=>isolated.close(),e=>e.code==='PARITY_DRAIN_TIMEOUT');assert.equal(isolated.db.prepare('SELECT 1 ok').get().ok,1)
});
...
test('executor status merevalidasi, mengirim tepat satu mutation, dan menyimpan SUCCEEDED',async()=>{
 const u=app.services.users.create({...sales,email:'execute@example.invalid',phone:'081233300008',salesCode:'EXE008',status:'ACTIVE'}),s=await connectAssist(u),headers={cookie:s.cookie,'x-csrf-token':s.login.body.csrfToken,'idempotency-key':'execute-preview-01'},preview=await request(base,'/api/assist/mutations/status/preview',{method:'POST',headers,body:JSON.stringify({prospectId:'star-p1',toStatus:'DEAL',reason:'Customer menyetujui pembelian'})});assert.equal(preview.status,201);assert.equal((await request(base,`/api/assist/mutations/${preview.body.operation.id}/confirm`,{method:'POST',headers,body:'{}'})).status,200);let mutations=0;app.services.mutations.setClient({updateProspectStatus:async(token,input)=>{mutations++;assert.equal(token,'synthetic-worker-access-token');assert.equal(input.prospectId,'star-p1');fakeReadClient.listProspects=async()=>[{id:'star-p1',status:'DEAL'}];return{id:'star-p1',status:'DEAL'}}});const result=await app.services.mutations.run(preview.body.operation.id);assert.equal(result.status,'SUCCEEDED');assert.equal(mutations,1);assert.equal(app.db.prepare('SELECT status FROM assist_mutation_operations WHERE id=?').get(preview.body.operation.id).status,'SUCCEEDED');assert.equal((await app.services.mutations.run(preview.body.operation.id)).status,'SUCCEEDED');assert.equal(mutations,1)
});

test('executor tidak mutate saat source stale dan timeout tidak di-retry',async()=>{
 const u=app.services.users.create({...sales,email:'executor-guard@example.invalid',phone:'081233300009',salesCode:'EXG009',status:'ACTIVE'}),s=await connectAssist(u),make=async(key)=>{fakeReadClient.listProspects=async()=>[{id:'star-p1',status:'HOT'}];const p=await request(base,'/api/assist/mutations/status/preview',{method:'POST',headers:{cookie:s.cookie,'x-csrf-token':s.login.body.csrfToken,'idempotency-key':key},body:JSON.stringify({prospectId:'star-p1',toStatus:'DEAL',reason:'Pengujian executor aman'})});await request(base,`/api/assist/mutations/${p.body.operation.id}/confirm`,{method:'POST',headers:{cookie:s.cookie,'x-csrf-token':s.login.body.csrfToken},body:'{}'});return p.body.operation.id};let calls=0,stale=await make('executor-stale-01');fakeReadClient.listProspects=async()=>[{id:'star-p1',status:'DEAL'}];app.services.mutations.setClient({updateProspectStatus:async()=>{calls++;return{id:'star-p1',status:'DEAL'}}});assert.equal((await app.services.mutations.run(stale)).status,'STALE_SOURCE');assert.equal(calls,0);let timed=await make('executor-timeout-01');app.services.mutations.setClient({updateProspectStatus:async()=>{calls++;throw Object.assign(new Error('timeout'),{code:'ASSIST_TIMEOUT'})}});assert.equal((await app.services.mutations.run(timed)).status,'UNKNOWN');assert.equal((await app.services.mutations.run(timed)).status,'UNKNOWN');assert.equal(calls,1)
});

test('switch ON mengantrikan executor otomatis setelah confirm',async()=>{
 let upstreamStatus='HOT';const isolated=createApp({dbPath:':memory:',adminEmail:'queue-admin@example.invalid',adminPassword:'AdminPassword!123',assistMasterKey:Buffer.alloc(32,8).toString('base64'),mutationExecutionEnabled:true,assistReadClient:{listProspects:async()=>[{id:'qp1',status:upstreamStatus}],listLeads:async()=>[]},assistMutationClient:{updateProspectStatus:async()=>{upstreamStatus='DEAL';return{id:'qp1',status:'DEAL'}}}}),u=isolated.services.users.create({...sales,email:'queue-sales@example.invalid',phone:'081233300012',salesCode:'QUE012',status:'ACTIVE'});isolated.services.assist.store(u.id,{accessToken:'queue-token',expiresAt:'2030-01-01T00:00:00Z'});const srv=isolated.server;await new Promise(r=>srv.listen(0,'127.0.0.1',r));const url=`http://127.0.0.1:${srv.address().port}`,login=await request(url,'/api/login',{method:'POST',body:JSON.stringify({email:u.email,password:sales.password})}),cookie=login.headers.get('set-cookie').split(';')[0],headers={cookie,'x-csrf-token':login.body.csrfToken,'idempotency-key':'queue-auto-01'},preview=await request(url,'/api/assist/mutations/status/preview',{method:'POST',headers,body:JSON.stringify({prospectId:'qp1',toStatus:'DEAL',reason:'Auto queue test'})}),confirmed=await request(url,`/api/assist/mutations/${preview.body.operation.id}/confirm`,{method:'POST',headers,body:'{}'});assert.equal(confirmed.body.executionEnabled,true);let row;for(let n=0;n<50;n++){row=isolated.db.prepare('SELECT status FROM assist_mutation_operations WHERE id=?').get(preview.body.operation.id);if(row.status==='SUCCEEDED')break;await new Promise(r=>setTimeout(r,5))}assert.equal(row.status,'SUCCEEDED');await new Promise(r=>srv.close(r));await isolated.close()
});

test('revocation di tengah pre-read mencegah POST mutation',async()=>{
 const u=app.services.users.create({...sales,email:'revoke-exec@example.invalid',phone:'081233300013',salesCode:'REV013',status:'ACTIVE'}),s=await connectAssist(u),headers={cookie:s.cookie,'x-csrf-token':s.login.body.csrfToken,'idempotency-key':'revoke-exec-01'},p=await request(base,'/api/assist/mutations/status/preview',{method:'POST',headers,body:JSON.stringify({prospectId:'star-p1',toStatus:'DEAL',reason:'Uji cabut akses'})});await request(base,`/api/assist/mutations/${p.body.operation.id}/confirm`,{method:'POST',headers,body:'{}'});let release,calls=0;fakeReadClient.listProspects=async()=>new Promise(r=>{release=r});app.services.mutations.setClient({updateProspectStatus:async()=>{calls++;return{id:'star-p1',status:'DEAL'}}});const running=app.services.mutations.run(p.body.operation.id);await new Promise(r=>setTimeout(r,5));app.db.prepare('DELETE FROM assist_connections WHERE user_id=?').run(u.id);release([{id:'star-p1',status:'HOT'}]);const result=await running;assert.equal(result.status,'FAILED');assert.equal(result.errorCode,'AUTH_REVOKED');assert.equal(calls,0)
});

test('startup ON memulihkan CONFIRMED yang belum pernah diklaim',async()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'mutation-recovery-')),dbPath=path.join(dir,'portal.db'),key=Buffer.alloc(32,9).toString('base64'),off=createApp({dbPath,adminEmail:'recover-admin@example.invalid',adminPassword:'AdminPassword!123',assistMasterKey:key,assistReadClient:{listProspects:async()=>[{id:'rp1',status:'HOT'}],listLeads:async()=>[]}}),u=off.services.users.create({...sales,email:'recover-sales@example.invalid',phone:'081233300014',salesCode:'REC014',status:'ACTIVE'});off.services.assist.store(u.id,{accessToken:'recover-token',expiresAt:'2030-01-01T00:00:00Z'});const t=new Date().toISOString(),op='recover-op-1';off.db.prepare('INSERT INTO assist_mutation_operations VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(op,u.id,'recover-key-01','STATUS_UPDATE','rp1','HOT','DEAL','Recovery aman','CONFIRMED',new Date(Date.now()+300000).toISOString(),t,t,t,null,null);await off.close();let status='HOT',calls=0;const on=createApp({dbPath,adminEmail:'recover-admin@example.invalid',adminPassword:'AdminPassword!123',assistMasterKey:key,mutationExecutionEnabled:true,assistReadClient:{listProspects:async()=>[{id:'rp1',status}],listLeads:async()=>[]},assistMutationClient:{updateProspectStatus:async()=>{calls++;status='DEAL';return{id:'rp1',status}}}});let row;for(let n=0;n<100;n++){row=on.db.prepare('SELECT status FROM assist_mutation_operations WHERE id=?').get(op);if(row.status==='SUCCEEDED')break;await new Promise(r=>setTimeout(r,5))}assert.equal(row.status,'SUCCEEDED');assert.equal(calls,1);await on.close();fs.rmSync(dir,{recursive:true,force:true})
});

test('status operasi mutation hanya dapat dibaca owner',async()=>{
 const u1=app.services.users.create({...sales,email:'op-owner@example.invalid',phone:'081233300010',salesCode:'OPO010',status:'ACTIVE'}),u2=app.services.users.create({...sales,email:'op-other@example.invalid',phone:'081233300011',salesCode:'OPO011',status:'ACTIVE'}),s1=await connectAssist(u1),s2=await connectAssist(u2),p=await request(base,'/api/assist/mutations/status/preview',{method:'POST',headers:{cookie:s1.cookie,'x-csrf-token':s1.login.body.csrfToken,'idempotency-key':'operation-read-01'},body:JSON.stringify({prospectId:'star-p1',toStatus:'DEAL',reason:'Membaca status operasi'})});assert.equal((await request(base,`/api/assist/mutations/${p.body.operation.id}`,{headers:{cookie:s1.cookie}})).status,200);assert.equal((await request(base,`/api/assist/mutations/${p.body.operation.id}`,{headers:{cookie:s2.cookie}})).status,404)
});
...
test('Sales submit draft via preview-confirm owner-only, CSRF, dan create tepat sekali',async()=>{
 const u=app.services.users.create({...sales,email:'submit-route@example.invalid',phone:'081211122233',salesCode:'SUB001',status:'ACTIVE'}),login=await request(base,'/api/login',{method:'POST',body:JSON.stringify({email:u.email,password:sales.password})}),cookie=login.headers.get('set-cookie').split(';')[0],jwt=`x.${Buffer.from(JSON.stringify({roles:JSON.stringify([{ConnectionType:'BusinessArea',ConnectionId:'channel-route',ConnectionName:'ASTRA MOTOR PENAJAM'}]),exp:1893456000})).toString('base64url')}.x`;app.services.assist.store(u.id,{accessToken:jwt,expiresAt:'2030-01-01T00:00:00.000Z'});const draft=await request(base,'/api/prospects',{method:'POST',headers:{cookie,'x-csrf-token':login.body.csrfToken},body:JSON.stringify({customerName:'Draft Kirim',phone:'081277788899',level:'LOW'})});assert.equal(draft.status,201);const noCsrf=await request(base,`/api/prospects/${draft.body.prospect.id}/submit`,{method:'POST',headers:{cookie},body:JSON.stringify({idempotencyKey:'route-key-123'})});assert.equal(noCsrf.status,403);const preview=await request(base,`/api/prospects/${draft.body.prospect.id}/submit`,{method:'POST',headers:{cookie,'x-csrf-token':login.body.csrfToken},body:JSON.stringify({idempotencyKey:'route-key-123'})});assert.equal(preview.status,201);assert.equal(preview.body.operation.status,'PREVIEW');const confirm=await request(base,`/api/prospects/${draft.body.prospect.id}/submit/${preview.body.operation.id}`,{method:'POST',headers:{cookie,'x-csrf-token':login.body.csrfToken},body:JSON.stringify({action:'CONFIRM'})});assert.equal(confirm.body.operation.status,'SUCCEEDED');assert.equal(mutationCalls.filter(x=>x[0]==='create').length,1);const status=await request(base,`/api/prospects/${draft.body.prospect.id}/submit/${preview.body.operation.id}`,{headers:{cookie}});assert.equal(status.body.operation.starNumber,'H704-NEW-1')
});
